...
BEGIN
  ...
  merged_by_phone AS (
    SELECT
      ...
      COALESCE(agg.masters_count, 0)::INT  AS masters_count,
      COALESCE(agg.masters, '[]'::jsonb)   AS masters,
      GREATEST(tg.tg_registered_at, agg.last_master_at) AS activity_at
    FROM phones p
    LEFT JOIN LATERAL (
  ...
  -- 5) Клиенты без телефона — отдельными строками
  merged_no_phone AS (
    SELECT
  ...
  page_rows AS (
    SELECT COALESCE(
      jsonb_agg(to_jsonb(p) ORDER BY p.activity_at DESC NULLS LAST),
      '[]'::jsonb
    ) AS rows
    FROM paged p
  )
